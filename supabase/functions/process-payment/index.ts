...
function isCpfValid(cpf: string) {
  const cleanedCpf = formatNumber(cpf);
...
serve(async (req) => {
  ...
  try {
    ...
    if (!paymentMethod || !amount || !customerData) {
      return new Response(JSON.stringify({ error: 'Dados incompletos. Informe método, valor e cliente.' }), {
        status: 400,
        headers: { 'Content-Type': 'application/json', ...corsHeaders },
      });
    }
    ...
    Object.keys(expfypayBody).forEach(key => {
      if (expfypayBody[key] === undefined) delete expfypayBody[key];
    });

    if (paymentMethod === 'CARD') {
      if (!cardToken) {
        return new Response(JSON.stringify({ error: 'Token de cartão ausente.' }), {
          status: 400,
          headers: { 'Content-Type': 'application/json', ...corsHeaders },
        });
      }
      ...
      try {
        ...
        return new Response(JSON.stringify({ error: 'Erro ao decodificar token do cartão.' }), {
          status: 400,
          headers: { 'Content-Type': 'application/json', ...corsHeaders },
        });
      }
